...
for N in SEQ_LENS:
    ...
    BLOCK_M = 64  # Must match the BLOCK_M used in tiled_attention_kernel

    flops = compute_flops(N, D_K)
    total_bytes = compute_bytes(N, D_K, BLOCK_M)
...
ax.loglog(ais, tflops, 'ro', markersize=10, label='Tiled Triton kernel')
